...
(function () {
  ...
  function favorFormHTML() {
    ...
    const title = editing ? '✏️ 修改随礼' : (recip ? '🔁 回礼 · 我随礼给「' + (recip.party || '') + '」' : (recipName ? '🔁 回礼 · 我随礼给「' + recipName + '」' : ''));
    const showTitle = editing || recip || recipName;
    const addRowBtn = single ? '' : `<button type="button" class="btn ghost sm mt" data-action="favor-add-row">+ 加一行</button>`;
    return `<div class="card">${showTitle ? `<h3>${title}</h3>` : ''}
